$(function(){
    ...
    $(".machine li a").click(function(){
        $(".machine li span").removeClass("acts");
        $(this).children("span").addClass("acts")
    });

    $(".rad3 input").on("input",function(){
        this.value = this.value.replace(/\D/g,"")
        var val = $(".rad3 input").val();
        var total1 = val * 80;
        var total2 = val *100;
        // var val = parseInt(val);
        if( val == ""){
           $(".rad_number").text("价格计算出错!"); 
           $(".rad_number").css("display","block");
           $(".rad_num1").text("0");
           $(".rad_num2").text("0");
        }
        if( val < 5 && val != ""){
            $(".rad_number").text("最少5用户起售!");
            $(".rad_number").css("display","block");
            $(".rad_num1").text("0");
            $(".rad_num2").text("0");
        }
        if( val >= 5 && val <= 500){
            $(".rad_number").css("display","none");
            $(".rad_num1").text(total1);
            $(".rad_num2").text(total2);
        }
        if( val > 500){
            $(".rad_number").text("用户数过大，请直接在线客服进行询价!");
            $(".rad_number").css("display","block");
            $(".rad_num1").text("0");
            $(".rad_num2").text("0");
        }
    ...
    $(".phone").hover(function(){
    	$(".right_p").animate({width:"200px",opacity:"1"},300)
    },function(){
    	$(".right_p").animate({width:"0",opacity:"0"},300)
    })
