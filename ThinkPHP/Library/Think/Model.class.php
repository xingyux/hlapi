...
class Model {
    ...
    public function getDbFieldsInfo() {
		...
		if(C('DB_FIELDS_CACHE')) {
			$fields = F('_fields/'.strtolower($db.'.'.$table.'.info'));
			
		}
		if(!$fields){
			$fields=$this->query("SELECT COLUMN_NAME as name,COLUMN_COMMENT as comment FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = '{$table}' AND TABLE_SCHEMA = '{$db}'");
			$fields = array_column($fields,"comment","name");
			F('_fields/'.strtolower($db.'.'.$table.'.info'),$fields);
		}
        return $fields;
    }			
    ...
    public function setTable($table) {
			$prefix =   $this->tablePrefix;
            $this->options['table'] =   $prefix.parse_name($table);
