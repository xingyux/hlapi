...
class SeoController extends HomeController {
    public function index(){
		$servers_where["servertype"]="seo";
		...
		if(!empty($_GET["room"])&&!empty($_GET["id"])){
			$this->display($_GET["room"].$_GET["id"]);die;
		}
		if(!empty($_GET["room"])){
			$this->display($_GET["room"]);die;
		}
		$this->display();die;
